...
import type { LandingPageFooter, SocialLink } from "@/src/types";

interface FooterProps {
  data?: LandingPageFooter | null;
}
...
function renderSocialIcon(link: SocialLink) {
  ...
  return (
    <div className="text-xs font-semibold">
      {link.platform?.slice(0, 2).toUpperCase() || "SM"}
    </div>
  );
}
...
export function Footer({ data }: FooterProps) {
  ...
  const logoAlt = data?.logo?.alt ?? "Montera Group Logo";
  const taglines =
  ...
  const newsletterPlaceholder = data?.newsletter?.placeholder ?? "Input email";
  const newsletterLabel = data?.newsletter?.cta_label ?? "Get voucher";
  const contactPhone = data?.contact?.phone ?? "+62-21-23093070";
  const contactEmail = data?.contact?.email ?? "Info@monteragoup.com";
  const addresses = data?.contact?.addresses?.length
    ? data.contact.addresses
    : [
        "Rukan Fresh Market No. C16 Green Lake City, Cipondoh Kota Tangerang, Banten Indonesia"
      ];
  const infoLinks = data?.info_links?.length
    ? data.info_links
    : [
  ...
  const copyrightTemplate =
    data?.copyright?.text_template ??
    "(c) {{year}} copyright by Montera Group";
